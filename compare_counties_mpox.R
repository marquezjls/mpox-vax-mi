...
df_hiv_counties <- df_hiv_counties %>% 
  ...
  mutate(cnt = ifelse(cnt == "Data suppressed", NA,cnt),
         cnt = as.double(cnt))
...
df_all <- df_all %>% 
  ...
  mutate(pct_first_dose = first_dose/adj_total_cnt,
...
df_all %>% 
  filter(total_cnt > 0) %>%
  ggplot(aes(x = fct_reorder(County,-pct_first_dose), y = pct_first_dose, fill = County)) +
  geom_point(aes(y = pct_first_dose, size= adj_total_cnt), alpha = 0.5, shape = 21) +
  geom_segment(aes(x = fct_reorder(County,-pct_first_dose), xend= fct_reorder(County,-pct_first_dose), y=0, yend=pct_first_dose))+
...
df_all %>% 
  filter(total_cnt > 0) %>%
  ggplot(aes(x = fct_reorder(County,-adj_total_cnt), y = pct_first_dose, fill = County)) +
  geom_point(aes(y = pct_first_dose, size= adj_total_cnt), alpha = 0.5, shape = 21) +
...
pop_tract <- get_acs(geography = "county", variables = "B01003_001E", state = "MI", 
                     geometry = T)
